...
public class EmailGeneratorTest extends BaseLogicTest {
        ...
        public void testGenerateFeedbackSessionEmails() throws Exception {
                FeedbackSessionAttributes session = fsLogic.getFeedbackSession("First feedback session",
                                "idOfTypicalCourse1");

                CourseAttributes course = coursesLogic.getCourse(session.getCourseId());

                StudentAttributes student1 = studentsLogic.getStudentForEmail(course.getId(),
                ...
                InstructorAttributes instructor1 = instructorsLogic.getInstructorForEmail(course.getId(),
                ...
                InstructorAttributes instructorToNotify = instructorsLogic.getInstructorForGoogleId(
                                session.getCourseId(),
                ...
                verifyEmailReceivedCorrectly(emails, student1.getEmail(), subject,
                                "/sessionOpeningEmailForStudent.html");
                verifyEmailReceivedCorrectly(emails, instructor1.getEmail(),
                                EmailWrapper.EMAIL_COPY_SUBJECT_PREFIX + subject,
                ...
                verifyEmailReceivedCorrectly(emails, student1.getEmail(), subject,
                                "/sessionReminderEmailForStudent.html");
                ...
                // 2 instructors and 3 students with deadline extensions do not need to be
                // notified
                ...
                verifyEmailReceivedCorrectly(emails, student1.getEmail(), subject,
                                "/sessionClosingEmailForStudent.html");
                verifyEmailReceivedCorrectly(emails, instructor1.getEmail(),
                                EmailWrapper.EMAIL_COPY_SUBJECT_PREFIX + subject,
                ...
                // this instructor email has been given co-owner privileges in the test file and
                // has joined
                InstructorAttributes coOwnerJoined = instructorsLogic.getInstructorForEmail(course.getId(),
                                "instructor1@course1.tmt");

                assertTrue(coOwnerJoined.hasCoownerPrivileges());

                verifyEmailReceivedCorrectly(emails, coOwnerJoined.getEmail(), subject,
                                "/sessionClosedEmailForCoOwner.html");
                ...
                // this instructor email has been given co-owner privileges in the test file but
                // has not joined
                InstructorAttributes coOwnerNotJoined = instructorsLogic.getInstructorForEmail(course.getId(),
                ...
                verifyEmailReceivedCorrectly(emails, student1.getEmail(), subject,
                                "/sessionPublishedEmailForStudent.html");
                verifyEmailReceivedCorrectly(emails, instructor1.getEmail(),
                                EmailWrapper.EMAIL_COPY_SUBJECT_PREFIX + subject,
                ...
                verifyEmailReceivedCorrectly(emails, student1.getEmail(), subject,
                                "/sessionUnpublishedEmailForStudent.html");
                verifyEmailReceivedCorrectly(emails, instructor1.getEmail(),
                                EmailWrapper.EMAIL_COPY_SUBJECT_PREFIX + subject,
                ...
                verifyEmail(email, student1.getEmail(), subject,
                                "/summaryOfFeedbackSessionsOfCourseEmailForStudent.html");
                ...
                subject = String.format(EmailType.STUDENT_COURSE_LINKS_REGENERATED.getSubject(), course.getName(),
                                course.getId());
                ...
                email = emailGenerator.generateFeedbackSessionSummaryOfCourse(
                                session.getCourseId(), instructor1.getEmail(),
                ...
                InstructorAttributes unregisteredInstructor = instructorsLogic.getInstructorForEmail(
                                "idOfTypicalCourse1",
                                "instructorNotYetJoinedCourse1@email.tmt");

                email = emailGenerator.generateFeedbackSessionSummaryOfCourse(
                                session.getCourseId(), unregisteredInstructor.getEmail(),
                                EmailType.INSTRUCTOR_COURSE_LINKS_REGENERATED);
                subject = String.format(EmailType.INSTRUCTOR_COURSE_LINKS_REGENERATED.getSubject(),
                                course.getName(), course.getId());
                ...
                FeedbackSessionAttributes notAnswerableSession = fsLogic
                                .getFeedbackSession("Not answerable feedback session", "idOfTypicalCourse2");
        ...
        public void testGenerateFeedbackSessionEmails_testUsersWithDeadlineExtensions() throws Exception {
                ...
                DeadlineExtensionAttributes instructor2 = dataBundle.deadlineExtensions
                                .get("instructor2InCourse1Session1");
                DeadlineExtensionAttributes instructor3 = dataBundle.deadlineExtensions
                                .get("instructor3InCourse1Session1");

                List<DeadlineExtensionAttributes> deadlineExtensions = List.of(student2, student4, student5,
                                instructor2, instructor3);

                ______TS("Feedback session closing alerts for users with deadline extensions");

                List<EmailWrapper> emails = emailGenerator.generateFeedbackSessionClosingWithExtensionEmails(session,
                                deadlineExtensions);
                ...
                verifyEmail(emails.get(0), instructor2.getUserEmail(), subject,
                                "/deadlineExtensionGivenInstructor.html");

                ______TS("Deadline extension updated for student");

                emails = emailGenerator.generateDeadlineUpdatedEmails(course, session,
                                Map.of(student2.getUserEmail(), newEndTime),
                                Map.of(student2.getUserEmail(), originalEndTime), false);
                ...
                emails = emailGenerator.generateDeadlineUpdatedEmails(course, session,
                                Map.of(instructor2.getUserEmail(), newEndTime),
                                Map.of(instructor2.getUserEmail(), originalEndTime), true);
                subject = String.format(EmailType.DEADLINE_EXTENSION_UPDATED.getSubject(),
                                course.getName(), session.getFeedbackSessionName());

                verifyEmail(emails.get(0), instructor2.getUserEmail(), subject,
                                "/deadlineExtensionUpdatedInstructor.html");
                ...
                verifyEmail(emails.get(0), instructor2.getUserEmail(), subject,
                                "/deadlineExtensionRevokedInstructor.html");
        }
        ...
        public void testGenerateFeedbackSessionEmails_testSanitization() throws Exception {
                ...
                CourseAttributes course = coursesLogic.getCourse(session.getCourseId());
                StudentAttributes student1 = studentsLogic.getStudentForEmail(course.getId(),
                                "normal@sanitization.tmt");
                InstructorAttributes instructor1 = instructorsLogic.getInstructorForEmail(course.getId(),
                                "instructor1@sanitization.tmt");
        ...
        public void testGenerateInstructorJoinEmail() throws Exception {
                ...
                subject = String.format(EmailType.INSTRUCTOR_COURSE_JOIN.getSubject(), course.getName(),
                                course.getId());
        ...
        public void testGenerateFeedbackSessionSummaryOfCourse_noSessionLinksFound() throws Exception {
                FeedbackSessionAttributes session = fsLogic.getFeedbackSession("Feedback session with no emails sent",
                                "idOfTestingNoEmailsSentCourse");

                CourseAttributes course = coursesLogic.getCourse(session.getCourseId());

                StudentAttributes noLinksStudent = studentsLogic.getStudentForEmail(course.getId(),
                                "student1@noemailssent.tmt");
                ...
                String subject = String.format(EmailType.STUDENT_EMAIL_CHANGED.getSubject(), course.getName(),
                                course.getId());
                ...
                email = emailGenerator.generateFeedbackSessionSummaryOfCourse(
                                session.getCourseId(), noLinksInstructor.getEmail(),
                                EmailType.INSTRUCTOR_COURSE_LINKS_REGENERATED);
                subject = String.format(EmailType.INSTRUCTOR_COURSE_LINKS_REGENERATED.getSubject(),
                                course.getName(), course.getId());
        ...
        public void testGenerateInstructorJoinEmail_testSanitization() throws Exception {
                ...
                InstructorAttributes instructor1 = instructorsLogic
                                .getInstructorForEmail("idOfTestingSanitizationCourse", "instructor1@sanitization.tmt");
                ...
                EmailWrapper email = emailGenerator
                                .generateNewInstructorAccountJoinEmail(instructor1.getEmail(), instructor1.getName(),
                                                joinLink);
                ...
                verifyEmail(email, instructor1.getEmail(), subject,
                                "/instructorNewAccountEmailTestingSanitization.html");
                assertEquals(email.getBcc(), Config.SUPPORT_EMAIL);
                ...
                verifyEmail(email, instructor1.getEmail(), subject,
                                "/instructorCourseJoinEmailTestingSanitization.html");
        ...
        public void testGenerateStudentCourseJoinEmail() throws Exception {
                ...
                StudentAttributes student = StudentAttributes.builder("", "student@email.tmt")
                ...
                String subject = String.format(EmailType.STUDENT_COURSE_JOIN.getSubject(), course.getName(),
                                course.getId());
                ...
                verifyEmail(email, student.getEmail(), subject,
                                "/studentCourseWithCoOwnersRejoinAfterGoogleIdResetEmail.html");
                ...
                verifyEmail(email, student.getEmail(), subject,
                                "/studentCourseWithoutCoOwnersRejoinAfterGoogleIdResetEmail.html");
        }
        ...
        public void testGenerateUserCourseRegisterEmail() throws Exception {
                ...
                EmailWrapper email = emailGenerator.generateUserCourseRegisteredEmail(name, emailAddress, googleId,
                                false, course);
